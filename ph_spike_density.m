function [SD,bins,SD_VAR,SD_SEM]=ph_spike_density(t_at,t_st,wn,keys,baseline,norm_factor)
...
if nargout>2 || (numel(baseline>=2) && (any(diff(baseline)) || any(diff(norm_factor))))
    SD_ms=zeros(numel(t_at),numel(PSTH_ms));
    for t=1:numel(t_at)
        idx_sta=t_st(t).states==sta;
        if any(idx_sta)
            arrival_times=t_at(t).arrival_times-t_st(t).states_onset(idx_sta);
            n_trials=n_trials+1;
    ...
    for t=1:numel(t_at)
        idx_sta=t_st(t).states==sta;
        if any(idx_sta)
            arrival_times=[arrival_times; t_at(t).arrival_times-t_st(t).states_onset(idx_sta)];
            n_trials=n_trials+1;
        end
    end
    if n_trials==0
