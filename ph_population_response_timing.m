...
for t=1:size(type_effectors,1) %typ=unique(per_trial.types)
    ...
    for g=1:numel(unique_group_values)
        ...
        for u=units
            ...
            %clear trpar
            for w=1:size(condition_matrix,1)/2
                clear trpar
                ...
                for par=1:numel(condition_parameters)
                    ...
                    trpar(par,:)=[pop.trial.(fn)]==condition_matrix(w,par); %condition_matrix_wohf?
                end
                ...
                for f=1:numel(u_hemifields) %hemifield (as compared to positions!
                    ...
                    if any(trtemp(tr_con)) % epoch averages for specific conditions
                        condition(tya,c).per_hemifield(f).unit(u).epoch_averages=...
                            nanmean(reshape([per_epoch(trtemp(tr_con),:).FR],size(per_epoch(trtemp(tr_con),:))),1);
                    else
                        condition(tya,c).per_hemifield(f).unit(u).epoch_averages=NaN(size(vertcat(pop.trial(1).epoch)));
                    end
                    ...
                    for wn=1:size(keys.PSTH_WINDOWS,1)
                        ...
                        for tr=1:numel(trials_for_SD)
                            condition(tya,c).per_hemifield(f).window(wn).unit(u).average_spike_density(tr,:)=...
                                ph_spike_density(pop.trial(trials_for_SD(tr)),wn,keys,0,1);
                        end
                        if isempty(trials_for_SD)
                            
                            condition(tya,c).per_hemifield(f).window(wn).unit(u).average_spike_density(1,:)=...
    ...
    for comp=1:numel(comparisons_per_effector)
        ...
        for eff=u_effectors
            ...
            for u=1:numel(units)
                ...
                for wn=1:numel(current(1).window) %% epochs might be different for different effectors.......... !!!!====???
                    %for g=1:numel(unique_group_values)
            ...
            keys=ph_get_epoch_keys(keys,typ,eff,sum(type_effectors(:,1)==typ)>1);%% does it make sense to distinguish by effector?
            ...
            %n_bins_disregarded_end          =(comparisons_per_effector(comp).order_onset{3}-keys.PSTH_WINDOWS{wo(1),4})/keys.PSTH_binwidth*-1;
            ...
            for u=1:numel(units)
                %tuning_onset(u)=find(~isnan([concatinated_after_onset(u,:)]),1); % in bins?
                ...
                %tuning_end(u)  =find( isnan([concatinated_after_onset(u,tuning_onset(u):end-1) NaN]),1)+tuning_onset(u)-1; % in bins?
            ...
            %tuning_direction(tuning_onset>size(concatinated_after_onset,2)-n_bins_disregarded_end)=NaN;
            ...
            %sigbins(t).per_effector(n).comparison(comp).tuning_end=tuning_end;%+comparisons_per_effector(comp).order_onset{2};
...
for t=1:numel(sigbins)
    ...
    for eff=u_effectors
        column=column+1;
        keys=ph_get_epoch_keys(keys,typ,eff,sum(type_effectors(:,1)==typ)>1);%% does it make sense to distinguish by effector?
        %conditions_to_loop=find(conditions_hf(:,2)==eff);
        
        n=find(u_effectors==eff);
        
        %             n_units=[];
        %             legend_label_indexes=[];
        %             legend_line_handles=[];
        for comp=1:numel(comparisons_per_effector)
            subplot(numel(comparisons_per_effector),numel(u_effectors),(comp-1)*numel(u_effectors)+column)
            hold on
    ...
    for eff=u_effectors
        ...
        for comp=1:numel(comparisons_per_effector)
            ...
            tr=[all_trialz.type]==typ & [all_trialz.effector]==eff & [all_trialz.completed]==1;
    ...
    for eff=u_effectors
        ...
        for comp=1:numel(comparisons_per_effector)
            ...
            %bins=min([to_plot1 to_plot2]):max([to_plot1 to_plot2]);
