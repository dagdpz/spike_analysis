function ph_target_preference(population,modified_keys)
...
idx_group_parameter=DAG_find_column_index(tuning_per_unit_table,keys.PR.group_parameter);
idx_unitID=DAG_find_column_index(tuning_per_unit_table,'unit_ID');
idx_RF_frame=DAG_find_column_index(tuning_per_unit_table,keys.PR.RF_frame_parameter);
group_values=tuning_per_unit_table(:,idx_group_parameter);
group_values=cellfun(@num2str, group_values, 'UniformOutput', false);
cell_in_any_group=[false; ~ismember(group_values(2:end),keys.PR.group_excluded)];
...
keys.normalization_field='PR';
keys.WINDOWS_PER_TYPE=keys.PR.WINDOWS_PER_TYPE;
...
regressors=keys.PR.regressors;
solution=keys.PR.solution;
...
for u=1:numel(PSTHs_per_unit)
    for w=1:numel(PSTHs_per_unit(u).window)
        clear reg4fit reg_tmp sol_tmp reg_perbin reg_bootstrapped sol_bootstrapped corrR1_bt
        for r=1:numel(regressors)
        ...
        if any(any(isnan(reg4fit))) || any(isnan(sol4fit))
            per_unit(u).window(w).betas=zeros(size(regressors))';
            per_unit(u).window(w).intercept=0;
            per_unit(u).window(w).pval=ones(size(regressors))';
            per_unit(u).window(w).in=[false false];
            per_unit(u).window(w).SS=NaN;
            per_unit(u).window(w).SSres=NaN;
            per_unit(u).window(w).corrR=corrR;
            per_unit(u).window(w).corrP=corrP;
        else
            [per_unit(u).window(w).betas,SE,PVAL,in,stats,nextstep,history] = stepwisefit(reg4fit,sol4fit,'display','off');
...
for spn=1:numel(sph)
    ...
    ph_PSTH_background(all_trialz(tr),y_lim(spn,:),y_lim(spn,:),y_lim(spn,:),keys,keys.PR.fontsize_factor)
...
for u=1:numel(per_unit)
    ...
    for spn=1:numel(sph)
        ...
        tr=[all_trialz.type]==typ & ismember([all_trialz.completed],keys.cal.completed) &...
            ismember([all_trialz.completed],keys.cal.completed) & ismember([all_trialz.reach_hand],u_con.reach_hand) & ismember([all_trialz.choice],u_con.choice);
        ph_PSTH_background(all_trialz(tr),y_lim,y_lim,y_lim,keys,keys.PR.fontsize_factor)
