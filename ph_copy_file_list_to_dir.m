function ph_copy_file_list_to_dir(example_list,project,versions, additional_pattern,remove_original,verbose)
...
for v=1:numel(versions)
    ...
    for e=1:numel(example_list)
        k = 1;
        todir=['Y:\Projects\' project '\ephys\' version '\' example_list(e).definition];
        
        for ex=1:numel(example_list(e).unit_IDs)
            name=example_list(e).unit_IDs{ex};
            if strfind(name,'*'),
                pattern = name;
            else
