...
for n_unit=1:numel(o)
    ...
    for b=1:numel(o(n_unit).trial)
        for n_time_arrival = 1:(numel(o(n_unit).trial(b).arrival_times)-1)
            ISI(n_unit).trial(b).isi(n_time_arrival)=(o(n_unit).trial(b).arrival_times(n_time_arrival+1) - (o(n_unit).trial(b).arrival_times(n_time_arrival)));
            
        end
    end
    all_ISI = cat(2,ISI(n_unit).trial.isi);
    
    if ~isempty(all_ISI)
        hist_values = hist(all_ISI,x_bins);
        perc_first_bin = (hist_values(1)/sum(hist_values))*100;
        hist(all_ISI,x_bins);
        h = findobj(gca,'Type','patch');
        set(h,'FaceColor','r','EdgeColor','w');
        x_bins_ticks = [10^-3 10^-2 10^-1 0.5];
        set(gca,'ytick',[max(hist(all_ISI,x_bins))],'xtick',x_bins_ticks,'box','off','xscale','log');%max(hist(all_ISI,x_bins)),'ycolor',[1 1 1]
        set(gca,'XTickLabel',sprintf('%2.0e|',x_bins_ticks));
        h2 = findobj(gca,'Type','line');
        set(h2,'Marker','none');
        xlim([0,0.7]);
        text(0.1,max(hist(all_ISI,x_bins)),[num2str(perc_first_bin,'%4.1f') '%<1ms'],'FontSize',8)
