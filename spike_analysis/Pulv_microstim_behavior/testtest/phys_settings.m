...
keys.epochs_to_plot_PSTH{2}                 ={'Facq','Fhol','Cue','PeriS','Thol'}';
...
keys.EPOCHS_PER_TYPE{3}={...
    ...
    'PeriS',	60,	-0.2,   0.05,	-0.01, 	0.05,   'INI';...
...
keys.epochs_to_plot_PSTH{3}                 ={'Facq','Fhol','Cue','MemE','MemL','PeriS','Thol'}';
...
keys.pop(cc).ylim                       = [-4 8.5];

cc=cc+1;% 10
keys.pop(cc).normalization              = 'none';
keys.pop(cc).group_parameter            = 'in_PeriS_epoch_Vsac_opt';
keys.pop(cc).epoch_RF                   = 'PeriS';
keys.pop(cc).conditions_to_plot         = {'Vsac'};
keys.pop(cc).FR_subtract_baseline       = 1;
keys.pop(cc).group_excluded             ={'','-','bi'};
