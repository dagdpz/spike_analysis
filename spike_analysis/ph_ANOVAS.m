        ...
        for a=1:numel(keys.position_and_plotting_arrangements)
            ...
            for type=unique(types)
                ...
                for comp=1:size(effectors_to_compare,2)
                    ...
                    tr_index= tr_considered & ismember(effectors,comp_eff) & types == type & control; %& ismember(hands,keys.cal.reach_hand);
    ...
    for hn=u_hnd
        ...
        for s=multicomp_epochs(:)'
            ...
            if any(~isnan(FR(tr)))
                ...
                if ch==2  && any(idx.in) %% there has to be choice and instructed
