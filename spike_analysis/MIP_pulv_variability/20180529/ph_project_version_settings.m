...
keys.batching.targets                   ={'PUL'};%'MIP',
keys.batching.monkeys                   ={'Tesla'};
%keys.Tesla.date                        ='[20160603 20180101]';
keys.Tesla.date                         ='[20160527 20170803]';
keys.Linus.date                         ='[20161103 20180101]';
% keys.Linus.sorted_neurons_sheetname     ='Linus';
% keys.Tesla.sorted_neurons_sheetname     ='Sheet1';
% keys.Linus.sorted_neurons_filename      ='Tes_sorted_neurons';
...
% cc=cc+1;
% keys.pop(cc).normalization              ='none';
% keys.pop(cc).group_parameter            = 'in_Cue_spaceLR_Ddre_han';
...
% cc=cc+1;
% keys.pop(cc).normalization              ='none';
% keys.pop(cc).group_parameter            = 'in_Del_spaceLR_Ddre_han';
% keys.pop(cc).conditions_to_plot         = {'Ddre'}; 
% keys.pop(cc).FR_subtract_baseline       = 1;
% 
% % 1
% cc=cc+1;
% keys.pop(cc).normalization              ='none';
% keys.pop(cc).group_parameter            = 'in_EDel_spaceLR_Ddre_han';
% keys.pop(cc).conditions_to_plot         = {'Ddre'}; 
% keys.pop(cc).FR_subtract_baseline       = 1;
% 
% % 1
