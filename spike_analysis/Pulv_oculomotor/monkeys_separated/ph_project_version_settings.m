...
keys.pop(cc).epoch_for_normalization  	= 'Fhol';
keys.pop(cc).conditions_to_plot       	= {'Msac'};
keys.pop(cc).tt.combine_tuning_properties       = {'tuning','in_Thol_spaceLR_Msac_opt','in_AH_Cue_position_Msac_opt'};
keys.pop(cc).tt.choices                    = [0];
keys.pop(cc).tt.hands                      = [0];
keys.pop(cc).plot_RF                 	= 1;
keys.pop(cc).RF_frame_colors                 	= {keys.colors.CS_IN,keys.colors.CS_CH,keys.colors.IS_IN,keys.colors.IS_CH,[127 127 127],[0 0 0]};
...
% to test
cs=cs+1;
keys.sct(cs).tt.tasktypes={'Msac'};
keys.sct(cs).X='in_AH_CS_Cue_epoch_FR_Msac_opt';
keys.sct(cs).Y='in_AH_CS_MemL_epoch_FR_Msac_opt';
...
keys.sct(cs).tt.combine_tuning_properties       = {'tuning','in_Thol_spaceLR_Msac_opt','in_AH_Cue_position_Msac_opt'};
...
cs=cs+1;
keys.sct(cs).tt.tasktypes={'Msac_opt'};
keys.sct(cs).X='in_CueG_spaceCI_IX_Msac_opt';
keys.sct(cs).Y='ch_CueG_spaceCI_IX_Msac_opt';
...
cs=cs+1;
keys.sct(cs).tt.tasktypes={'Msac_opt'};
keys.sct(cs).X='in_PreG_spaceCI_IX_Msac_opt';
keys.sct(cs).Y='ch_PreG_spaceCI_IX_Msac_opt';
...
cs=cs+1;
keys.sct(cs).tt.tasktypes={'Vsac_opt'};
keys.sct(cs).X='in_CueG_spaceCI_IX_Vsac_opt';
keys.sct(cs).Y='ch_CueG_spaceCI_IX_Vsac_opt';
keys.sct(cs).X_sig='in_CueG_spaceLR_Vsac_opt';
keys.sct(cs).Y_sig='ch_CueG_spaceLR_Vsac_opt';
keys.sct(cs).color_option='monkeys_by_marker';
keys.sct(cs).tt.choices=[0,1];
