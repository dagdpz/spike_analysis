...
%     end
    ...
    for te=1:size(type_effectors,1)
        ...
        for c= 1:size(condition_matrix,1) %% not sure if dom here is 1
            ...
            clear per_pos mean_RTs mean_dur mean_amp mean_vel n_trials n_trials_l
            for s=1:numel(data)
                ...
                n_trials(s)=sum(te_idx);
                n_trials_l(s)=sum(real([saccades.tar_pos]-[saccades.fix_pos])<0);
                ...
                for f=1:numel(fixations)
                    ...
                    for p=1:numel(positions)
                        ...
                        per_pos(f,p).n_trials(s)=sum(tr_idx);
                        per_pos(f,p).percent_trials(s)=sum(tr_idx)/n_trials(s)*100;
                    end
                end
            end
            ...
            for f=1:numel(fixations)
                ...
                for p=1:numel(positions)
                    ...
                    %patch(real(x0)+r*sin(angles), imag(x0)+r*cos(angles),'color',Colors(round(nanmean([per_pos(f,p).percent_trials]))+1,:));
            ...
            title([num2str(round(nanmean(n_trials_l/n_trials*100))) ' + ' num2str(round(sterr(n_trials_l./n_trials*100))) '% left']);
            axis equal
            colorbar;
