...
for unit=1:numel(population)
    ...
    for a=1:numel(keys.position_and_plotting_arrangements)
        ...
        for type=unique(types)
            %keys.several_effectors=numel(unique(effectors(types==type)))>1; %good idea, but we actually want several effectors in most cases --> move to general_settings
            %             keys.several_effectors=1;
            %             if keys.several_effectors
            %                 all_effectors={unique(effectors)};
            %                 effector_loop=1;
            %             else
            %                 all_effectors=num2cell(unique(effectors));
            %                 effector_loop=1:numel(all_effectors);
            %             end
            %
            %             for eff=effector_loop
            
            %% selection of epochs to plot
            %                 effectors_effector_loop=all_effectors{eff};
            %                [keys all_sta]=get_epoch_keys(keys,type,effectors_effector_loop,keys.several_effectors);
            ...
            for fig=unique_figures
                ...
                for e=1:numel(effectors_on_figure)
                    ...
                    subplots=unique([o.trial(of_index & [o.trial.effector]==effector).subplot_pos]);
                    ...
                    for sub=subplots
                        ...
                        for w=1:size(keys.PSTH_WINDOWS,1)
                            ...
                            for lin=unique_lines
                                ...
                                % PSTH
                                state_seperator(lin)=state_shift + t_after_state + 0.1;
                                [histo(lin,:) bins]=ph_spike_density(line_struct,w,keys,0,1);
                    ...
                    subplots=1:max([o.trial.subplot_pos]);
                    ...
                    for sta=all_sta
                        for lin=unique_lines
                            ...
                            plot_firing_rate_heatmap(FR_heat(sta,lin).pos,o.columns,o.rows,1:max(subplots));
                    ...
                    if keys.plot.polars_on_extra_figure
                        title_and_save(FR_summary_handle,plot_2_title,[plot_2_title fig_title_part],keys);
                        plot_2_title        =[fig_title ' polars'];
                        FR_summary_handle   =figure('units','normalized','outerposition',[0 0 1 1],'name',[plot_2_title fig_title_part]);
                    end
                    ...
                    title_and_save(FR_summary_handle,plot_2_title,[plot_2_title fig_title_part],keys);
                    %                         if keys.several_effectors; title_and_save(FR_summary_handle,plot_2_title,[plot_2_title fig_title_part],keys); end
                end
                
                %% Summary plot
               
                fig_title=sprintf('%s, %s, %s type, %s %s %s' ,keys.unit_ID, keys.target, type_string, keys.arrangement, title_part, title_value);
                plot_3_title        =[fig_title ' effector summary'];
                FR_summary_handle   =figure('units','normalized','outerposition',[0 0 1 1],'name',[plot_3_title fig_title_part]);
                        ...
                        for w=1:size(keys.PSTH_WINDOWS,1)
                            ...
                            for h=1:numel(unique_hemifields)
                                for lin=1:max(unique_lines)
                                    ...
                                    PSTH=ph_spike_density(o.trial(tr_index),w,keys,0,1);
                ...
                for r=1:numel(unique_rows)
                    row=unique_rows(r);
                    for c=unique_columns
%                         tr_index=of_index & [o.trial.effector]==effector & [o.trial.column]==c & [o.trial.accepted]==1;
%                         if sum(tr_index)==0
%                             continue;
%                         end
                        subplot(hs(r,c))
...
map = flipud(map);
% colormap(c);
% map = [1:4:256;zeros(1,64);zeros(1,64)]'/256; % red unicolor
colormap(map);
...
if keys.plot.polars_on_extra_figure % keys.several_effectors % debug if ==0
    n_rows_FR           =n_lines;
...
% else
%     n_rows_FR           =n_lines + 3*numel(effectors_on_figure);
%     n_rows_PSTH         =n_lines + 3*numel(effectors_on_figure);
%     FR_row_shift        =n_states*(lin+1) + 4*n_states*(lin-1);
% end
% if keys.plot.average_heat_maps % debug if ==0
%    n_rows_FR           =n_rows_FR + 2;
% end
switch subplot_case
    case 'PSTH'
