function ph_PSTH_background(trial,y_lim,y_limframe,y_limPSTH,keys,ff)
if nargin<5
    ff=1;
end
if diff(y_lim)==0
...
for w=1:size(keys.PSTH_WINDOWS,1)
    ...
    %here we loop, just so not everything is one text object later, which
    %is sort of a bug in export_fig
    ...
    for p=1:numel(state_positions)
        ...
        'fontsize',8*ff,'horizontalalignment','center','color',[p p p]/255); %this is a trick: becuase they are plotted in different colors, they will be diffrent elements in adobe
    end
    text(double(state_positions'),ones(size(state_positions'))*(y_limframe(2)-diff(y_limframe)*0.15),state_names(states_in_window),...
        'fontsize',6*ff,'interpreter','none','rotation',90,'verticalalignment','top');  %label for state
    ...
    'color',[1 0 1],'fontsize',6*ff,'horizontalalignment','center','verticalalignment','top');
    for ep=find(epochs_in_window')
        ...
        text(double(state_shift+relative_epochs(ep,1)+diff(relative_epochs(ep,:))/2),y_limPSTH(1)+diff(y_limPSTH)*0.05,epoch_names(ep),'color',[ep ep ep]/255,'verticalalignment','middle','horizontalalignment','center','fontsize',6*ff);
        uistack(ptch, 'bottom');
    end
    %rectangle('Position',[state_seperator y_limframe(1) t_after_state-t_before_state diff(y_limframe)]) %frame for indicating state separation
...
for k=1:numel(y_tick)
  text(-(state_seperator_max-0.1)/400, y_tick(k),num2str(y_tick(k)),'fontsize',8*ff,'horizontalalignment','right');
