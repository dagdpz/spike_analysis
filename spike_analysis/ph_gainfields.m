...
%significance_measure='unpaired';%'paired'; %'unpaired';
method='FR_vector'; %'FR_vector'; 'Von_mises_fit'; %
%method='Von_mises_fit'; %'FR_vector'; 'Von_mises_fit'; %


for fn=fieldnames(modified_keys)'
    keys.(fn{:})=modified_keys.(fn{:});
end
% keys.path_to_save=[keys.drive filesep keys.basepath_to_save filesep keys.project_version filesep 'tuning_curves' filesep];
% if ~exist(keys.path_to_save,'dir');
%     mkdir([keys.drive keys.basepath_to_save filesep keys.project_version ],'tuning_curves');
% end
...
%n_rows_columns=ceil(sqrt(n_epochs));

pct1 = 100*0.05/2; % alpha 0.05
pct2 = 100-pct1;

%gaussian
%F = @(a,x) a(1)+a(2)*exp(-(x-a(3)+2*pi*(x<(a(3)-pi))-2*pi*(x>(a(3)+pi))).^2/2*a(4)); %+2*pi*(x<(a(3)-pi))-2*pi*(x>(a(3)+pi)) this part is due to the circular shift
...
for u=1:N_units % %48%%90:N_units
    ...
    for current_epoch=1:n_epochs
        ...
        clear bootstraped  %bootstat1 bootstat2
        
        %% von mises fit parameters
        idx=~isnan(FRs);
        %         LB=double([max([abs(FRs);1])*-1.3 -pi    min(FRs)        1/pi]); %% Amplitude Direction Baseline Width
        %         UB=double([max([abs(FRs);1])*1.3  pi     max([FRs;1])    10000]);%% Amplitude Direction Baseline Width
        %         X0=double([0                      0      nanmean(FRs)    4/pi]); %% Amplitude Direction Baseline Width
...
%             [~,~,residuals]=lsqcurvefit(F,double(X0),double(angles(idx)),double(FRs(idx)),LB,UB,opts);
%             significant_RF=ttest(abs(residuals),double(abs(FRs(idx)-nanmean(FRs(idx)))),0.025,'left');
        ...
        for lin=1:max(unique_lines_effector)
            ...
            %
            %             %% von mises fit parameters
            %             current_trial_indexes=find(tr');
            %             current_FRs=FRs(current_trial_indexes);
            %             LB=double([0                        -pi     min(current_FRs)        1/pi]); %% Amplitude Direction Baseline Width
            %             UB=double([max([current_FRs;1])*1.3  pi     max([current_FRs;1])    10000]);%% Amplitude Direction Baseline Width
            %             X0=double([max(current_FRs)          0      min(current_FRs)        4/pi]); %% Amplitude Direction Baseline Width
            ...
            if min(N_per_pos_lin)>=keys.cal.min_trials_per_condition
                ...
                significant_fit(lin)=inmodel; %ttest(abs(residuals),double(abs(FRs(tr)-nanmean(FRs(tr)))),0.025,'left');
                %                 unit_valid_counter=unit_valid_counter+1;
            else
...
%                         case 'Von_mises_fit'
%                             bootstat(k,:)=lsqcurvefit(F,double(X0),double(angles(idx_idx(:))),double(FRs(idx_idx(:))),LB,UB,opts);
%                     end
                end
            else
                bootstat=single(NaN(n_iterations_bootstrap,4));
            end
            %                    bootstat1(:,lin) = bootstat(:,1);
            %                     bootstat2(:,lin) = mod(bootstat(:,2)-mean_FR_angle+5*pi,2*pi)-pi+mean_FR_angle;
            %                     bootstraped(lin).CI_amp   = [prctile(bootstat1(:,lin),pct1,1) nanmean(bootstat1(:,lin)) prctile(bootstat1(:,lin),pct2,1)];
            %                     bootstraped(lin).CI_angle = [prctile(bootstat2(:,lin),pct1,1) nanmean(bootstat2(:,lin)) prctile(bootstat2(:,lin),pct2,1)];
            ...
            %bootstraped(lin).mean_base  = nanmean(bootstat(:,3));
            ...
            %             bootstrapped_vectors_x=real(bootstat(:,1).*exp(1i*bootstat(:,2)));
            %             bootstrapped_vectors_y=imag(bootstat(:,1).*exp(1i*bootstat(:,2)));
            %             bootstrapped_vectors_mean=[nanmean(bootstrapped_vectors_x) nanmean(bootstrapped_vectors_y)];
            %             bootstrapped_vectors_mean=bootstrapped_vectors_mean./norm(bootstrapped_vectors_mean);
            %
            %             projected_vectors=bootstrapped_vectors_mean*[bootstrapped_vectors_x'; bootstrapped_vectors_y'];
            %
            %             angle(nansum(bootstat(:,1).*exp(1i*bootstat(:,2))))
            ...
            %                     if n_samples_per_bootstrap>0
            %                         for k=1:n_iterations_bootstrap
            %                             idx_idx=cellfun(@(x) x(randsample(numel(x),10,true)),idexes_per_position,'Uniformoutput',false);
            %                             idx_idx=[idx_idx{:}];
            %                             %bootstraped(lin,k).FR_vector=nanmean(FR_vectors(idx_idx(:)))
            %
            %                         end
            %                     else
            %                         bootstat=single(NaN(n_iterations_bootstrap,4));
        ...
        %% storing parameters
        %         population(u).RF_per_epoch(ep).von_mises_coefficients=coefficients;
        %         population(u).RF_per_epoch(ep).p_von_mises=p_von_mises(2);
    ...
    if do_plot
        %         all_bt=vertcat(RF_per_epoch.bootstraped);
        %         max_FR_vector=max(abs(vertcat(all_bt.FR_vectors)));
        %         max_FR=max(vertcat(per_epoch.FR));
        ...
        for current_epoch=1:n_epochs
            ...
            title_part=[keys.EPOCHS_PER_TYPE{type}{ep,1} 'val_RF_gain_shift ' num2str([SIG.unit_valid SIG.significant_RF SIG.significant_gain SIG.significant_shift])];
                    ...
                    %polar(0, max(abs(vertcat(bootstraped.FR_vectors))),linespec);
                    ...
                    for lin=1:max(unique_lines_effector)
                        ...
                        %plot(real(bootstraped(lin).CI_amp(2)*FR_vector),imag(bootstraped(lin).CI_amp(2)*FR_vector),'d','color',pop.PSTH_perpos_colors(lin,:));
            ...
            title_part=[keys.EPOCHS_PER_TYPE{type}{ep,1} ' tot_' num2str(SIG.significant_RF) '_or_' OR '_bl_' BL  '_re_' RE];
                    ...
                    for lin=1:max(unique_lines_effector)
                        ...
                        %coefficients=[bootstraped(lin).mean_amp bootstraped(lin).mean_angle bootstraped(lin).mean_base bootstraped(lin).mean_kappa];
                        ...
                        %FRlim(ep,:)=get(gca,'ylim');
                    end
            end
            %title({keys.EPOCHS_PER_TYPE{type}{ep,1}; num2str([SIG.significant_RF SIG.significant_gain SIG.significant_shift])});
...
%         %% von mises fit to current epochs FR!!
%         FRs=FRs(~isnan(FRs));angles_ep=angles(~isnan(FRs));
%         LB=double([min(FRs) 0 -pi 1/pi]);
%         UB=double([max([FRs;1]) max([FRs;1])*1.3 pi 10000]);
%         X0=double([min(FRs) max(FRs) 0 4/pi]);
%         if numel(FRs)>10
%             coefficients = lsqcurvefit(F,double(X0),double(angles_ep),double(FRs),LB,UB,opts);
%             allx=[ones(size(angles_ep)).*coefficients(1),F([0 coefficients(2:end)],angles_ep)];
%             [~,~,p_von_mises,~,~,~,~] = stepwisefit(allx,FRs,'display','off');
%             unique_FRs=unique([FRs;0]);
%             mean_FR_step=mean(diff(unique_FRs))/2;
%         else
%             coefficients=NaN(size(X0));
%             p_von_mises(2)=100;
%             mean_FR_step=1;
%         end
%         if isempty(mean_FR_step)
