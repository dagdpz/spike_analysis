...
%format_excel_tuning_table('W:\Projects\Pulv_microstim_behavior\ephys\ephys_analysis_v7_July2016_coordinates','tuning_table_mem_combined')
%
...
for t=1:numel(tasks)
    ...
    for c=1:numel(cases)
        ...
        %idx_N_trials   =find_column_index(tuning_per_unit_table,['trials_per_condition_' current_task '_' current_case]);
            ...
            for h=1:numel(hands)
                ...
                for s=1:numel(sides)
                    ...
                    for f=1:numel(general_factors_per_hand_space)
                        ...
                        if isempty(idx); continue; end;
                        temp_table51=[temp_table51 vertcat({[hands{h} '_' sides{s} '_' general_factors_per_hand_space{f} '_tuning']},tuning_per_unit_table(2:end,idx))];
                        %
                    end
                end
            end
