...
%                         RFsizes2                 =2*sqrt([Parameters_temp.sx2].*[Parameters_temp.sy2]);
            ...
            for u=1:numel(group_units)
                ...
                if ~isempty(idx_RF_frame)
                    ...
                    % keys.PO.RF_frame_colors                 	= {keys.colors.CS_IN,keys.colors.CS_CH,keys.colors.IS_IN,keys.colors.IS_CH,[127 127 270],[0 0 0]};
                    % keys.PO.RF_frame_entries                 	= {'CStrue','CSfalse','IStrue','ISfalse','-true','-false'};
