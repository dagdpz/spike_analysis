                ...
                for t=1:numel(keys.batching.targets)
                    ...
                    for pt=1:numel(keys.cal.perturbation_groups)
                        ...
                        for s=1:numel(sessions)
                            ...
                            blocks(idx,3)=runs;
                            filelist_formatted.([keys.monkey(1:3) '_' target '_PT' num2str(pt-1) '_' keys.tt.tasktypes{tt}]){s,1}=session;
                            filelist_formatted.([keys.monkey(1:3) '_' target '_PT' num2str(pt-1) '_' keys.tt.tasktypes{tt}]){s,2}=runs;
                        end
