...
for P=population_analysis_to_perform
    ...
    for cc=1:numel(keys_in.(ana))
        ...
        elseif ~isempty(trials)
            trials=ph_assign_perturbation_group(keys,trials);
        end
        population = ph_accept_trials_per_unit(population_full,trials,keys);
        
        
        for a=1:numel(keys.(AN).position_and_plotting_arrangements)
            keys.arrangement= keys.(AN).position_and_plotting_arrangements{a};
            if ~isempty(trials)
                [trials]=ph_arrange_positions_and_plots(keys,trials); %% not sure if this works
            end
            ...
            for condition_to_plot=keys_in.(ana)(cc).conditions_to_plot
                ...
                keys.tuning_table=keys_in.tuning_table;
                ...
                switch ana
                    ...
                    case 'tun'
                        %% THIS should not go through arrangements !
