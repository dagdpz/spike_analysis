...
for b=1:size(o_t,2)
    if isfield(o_t(b).trial(1),'TDT_LFPx') || isfield(o_t(b).trial(1),'TDT_MUAx') || isfield(o_t(b).trial(1),'TDT_ECG1')
        ...
        for c=1:n_chans_s
            ...
            if ~isempty(MUA)
            sites(s).MUA_samples   =[sites(s).MUA_samples MUA_samples];
            sites(s).MUA_tStart    =[sites(s).MUA_tStart o_t(b).trial.TDT_MUAx_tStart];
...
fieldnames_to_remove={'time_axis','x_eye','y_eye','x_hnd','y_hnd','unit','channel','TDT_LFPx','TDT_MUAx','TDT_CAP1','TDT_ECG1','TDT_ECG4','TDT_POX1'};
Trial=[o_t.trial];
...
fields_to_remove={'unit','channel','x_eye','y_eye','x_hnd','y_hnd','time_axis',...
    'TDT_LFPx','TDT_LFPx_SR','TDT_LFPx_tStart','TDT_MUAx','TDT_MUAx_SR','TDT_MUAx_tStart'};
