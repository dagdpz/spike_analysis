...
if keys.TU.redo_statistics
    ...
    writecell(excel_table, [keys.tuning_table_foldername 'tuning_table_' keys.target '_' keys.TU.unique_title '.xlsx'], 'FileType','spreadsheet') % if you have fieldtrip package installed, you will have name conflicts with a function from there 
    
end
    
    excel_table=ph_format_excel_tuning_table(keys.tuning_table,keys.conditions_to_plot(:)',keys);
    writecell(excel_table, [keys.basepath_to_save keys.project_version filesep 'tuning_table_' keys.target '_' keys.TU.unique_title '.xlsx'], 'FileType','spreadsheet')
