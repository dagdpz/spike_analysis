...
x=P(:,1);
y=P(:,2);
dx=min(diff(unique(x)))/2; % used for limits of peak: max half the min target out of target array
dy=min(diff(unique(y)))/2; % used for limits of peak: max half the min target out of target array
