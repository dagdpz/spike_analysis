...
sizes.NN=2*vertcat(tuning_per_unit_table{row_NN & row_size_valid3,ix_size_x});
...
[sig.siz_CHN, ps.siz_CHN]=ttest2(sizes.CH,sizes.NN);
[sig.siz_INN, ps.siz_INN]=ttest2(sizes.NN,sizes.IN);
...
means_sizes.NN=mean(sizes.NN);
std_sizes.CH=std(sizes.CH);
std_sizes.IN=std(sizes.IN);
std_sizes.NN=std(sizes.NN);
