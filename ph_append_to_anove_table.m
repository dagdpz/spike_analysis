    ...
    case 'regression'
        ...
        for u=1:numel(U)
            ...
            for w=1:numel(W)
                ...
                wn=C.name{:};
                if u==1
                    table(1,w+1:w+7)={[wn '_b1'],[wn '_b2'],[wn '_p1'],[wn '_p2'],[wn '_SS'],[wn '_corrP'],[wn '_corrR']};
                end
                table(u+1,w+1:w+7)={C.betas(1),C.betas(2),C.pval(1),C.pval(2),C.SS,C.corrP(1,2),C.corrR(1,2)}; %% SE(!)???
            end
        end
end
...
completed_table = DAG_update_mastertable_cell(tuning_per_unit_table,table_for_updating,rows_to_update);
